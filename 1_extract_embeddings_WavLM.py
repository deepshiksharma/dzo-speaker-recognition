...
def process_folder(folder, labeled):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc=os.path.basename(folder)):
...
index.to_csv(
    os.path.join(OUTPUT_DIR, "wavlm_embedding_index.csv"),
    index=False
)

print("\nwavlm embedding matrix:", all_embeddings.shape)
